...
def generate_hilo_map(csv_file='mouse_telemetry.csv', cell_size=30, wall_threshold=5, min_neighbors=2):
    try:
    ...
    if not map_pts_x:
        print("No valid wall points detected.")
        return

    x_min, x_max = min(map_pts_x) - 50, max(map_pts_x) + 50
    y_min, y_max = min(map_pts_y) - 50, max(map_pts_y) + 50
    
    x_bins = np.arange(x_min, x_max + cell_size, cell_size)
    y_bins = np.arange(y_min, y_max + cell_size, cell_size)

    counts, xedges, yedges = np.histogram2d(map_pts_x, map_pts_y, bins=[x_bins, y_bins])
    
    binary_grid = (counts >= wall_threshold).astype(int)

    kernel = np.array([[1, 1, 1],
                       [1, 0, 1],
                       [1, 1, 1]])
    
    neighbor_count = convolve(binary_grid, kernel, mode='constant', cval=0)
    
    final_mask = (binary_grid == 1) & (neighbor_count >= min_neighbors)
    final_grid = final_mask.astype(float)

    plt.figure(figsize=(10, 8), facecolor='#121212')
    ax = plt.gca()
    ax.set_facecolor('#121212')

    plt.plot(df['pos_x'], df['pos_y'], color='cyan', alpha=0.3, lw=1, label='Mouse Path')
    
    final_grid[final_grid == 0] = np.nan
    
    plt.pcolormesh(xedges, yedges, final_grid.T, cmap='Wistia', edgecolors='#333', linewidth=0.1)
    
    plt.title(f"Binary Maze Grid | Cell: {cell_size}mm | Threshold: {wall_threshold}", color='white')
    plt.xlabel("X (mm)", color='white'); plt.ylabel("Y (mm)", color='white')
    plt.tick_params(colors='white')
    plt.axis('equal')
    plt.gca().invert_yaxis() 
    
    plt.tight_layout()
    plt.savefig('square_maze_reconstruction.png', dpi=300, facecolor='#121212')
    plt.show()
...
def main():
    sim = Space(width=1200, height=1000, gravity=(0, 0), fps=12, caption="HLM Explorer")
    map_a = [      
    ...
    connections, sx, sy, spacing = map_a, 1, 3, 180
    
    maze_lines = build_synced_maze(sim, connections, spacing=spacing)
    grid = Grid(spacing, {'x':sx, 'y':sy})
    player = Mouse(sim, grid.start.pos, 50, 2.0, (50, 255, 150), maze_lines)
    ...
    generate_hilo_map(csv_file=csv_name, cell_size=30, wall_threshold=5, min_neighbors=2)
