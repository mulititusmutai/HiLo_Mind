...
def get_astar_path(nodes_dict, start_node, goal_node, treat_unknown_as_open=False):
    ...
    while open_set:
        ...
        if current.id == goal_node.id:            
            ...
            return path[::-1], g_score[goal_node.id]
    ...
    def decide_next(self, node: Node, goal_node: Node):
        ...
        for d in node.passages:
            ...
            if nbr and nbr.id not in self.visited:
                unvisited_options.append(nbr)

        if len(unvisited_options) > 1:
            best_nbr = None
            lowest_cost = float('inf')
            for nbr in unvisited_options:
        ...
        elif len(unvisited_options) == 1:
            return ('drive', unvisited_options[0])
...
class Tree:
    ...
    def _do_scan(self, state):
        node = self._at_node
        # Thresholds in sim units (halved from physical mm)
        short_thresh = 50  # ~190mm
        long_thresh = 140   # ~370mm

        # 1. Acquire both levels of spatial data
        short_walls = read_wall_data(state, self.mouse.mount_angles, open_threshold=short_thresh)
        long_walls = read_wall_data(state, self.mouse.mount_angles, open_threshold=long_thresh)
        
        opposites = {'n': 's', 's': 'n', 'e': 'w', 'w': 'e'}
        
        # 2. Process Short-Range (Immediate Ground Truth)
        for direction, is_wall in short_walls.items():
            node.walls[direction] = is_wall
            
            # Reciprocal Update for immediate neighbor
            neighbor = node.grid_links.get(direction)
            if neighbor:
                neighbor.walls[opposites[direction]] = is_wall

        # 3. Process Long-Range (Extended Insight)
        for direction, seen_as_wall in long_walls.items():
            # Logic Gate: Only mark a long wall if the immediate path is clear
            # and we actually detected something at the long range.
            if not short_walls[direction] and seen_as_wall:
                neighbor = node.grid_links.get(direction)
                if neighbor:
                    # Attribute the wall to the neighbor's far exit
                    neighbor.walls[direction] = True
                    
                    # Reciprocal Update for the node beyond the neighbor
                    node_beyond = neighbor.grid_links.get(direction)
                    if node_beyond:
                        node_beyond.walls[opposites[direction]] = True
                
        # 4. Standard DFS/FSM Logic
        open_passages = {d for d, wall in short_walls.items() if not wall}
...
class Space:
    ...
    def draw_debug_info(self, mouse):
        true_pos = mouse.body.position
        true_heading = math.degrees(mouse.body.angle) % 360
        est_pos = (mouse.est_x, mouse.est_y)
        est_heading = math.degrees(mouse.est_theta) % 360

        true_txt = f"ACTUAL: x:{true_pos.x:>6.1f} y:{true_pos.y:>6.1f} hdg:{true_heading:>5.1f}°"
        est_txt  = f"ESTIM : x:{est_pos[0]:>6.1f} y:{est_pos[1]:>6.1f} hdg:{est_heading:>5.1f}°"
        
        dist_err = math.dist((true_pos.x, true_pos.y), est_pos)
        err_txt  = f"DRIFT : {dist_err:.2f} px"

        self.screen.blit(self.font.render(true_txt, True, (255, 255, 255)), (10, 10))
        self.screen.blit(self.font.render(est_txt, True, (0, 255, 0)), (10, 30))
        self.screen.blit(self.font.render(err_txt, True, (255, 100, 100)), (10, 50))
    ...
    def draw(self):
        ...
        if self.camera_target:
            self.draw_debug_info(self.camera_target)
...
class SonarAgent:
    def __init__(self, rel_angle_deg, arm_length=0, fov=15, max_range=500):
    ...
    def _perform_scan(self, walls):
        ...
        # Simulation scale constants: 20mm physical -> 10 sim units, 400mm physical -> 200 sim units
        MIN_CAP = 10.0 
        MAX_CAP = 200.0

        for rel_angle in range(-self.fov//2, self.fov//2 + 1, 5):
            angle_rad = math.radians(base_deg + rel_angle)
            ray = LineString([self.origin, (self.origin[0] + self.max_range * math.cos(angle_rad), 
                                            self.origin[1] + self.max_range * math.sin(angle_rad))])
            
            # Initialize with MAX_CAP instead of max_range to enforce upper limit
            closest = MAX_CAP
            hit = None
            ...
            for wall in walls:
                if ray.intersects(wall):
                    ...
                    if isinstance(inter, Point):
                        ...
                        # Apply Lower Cap: If too close to read, treat as the minimum readable distance
                        if d < MIN_CAP:
                            d = MIN_CAP
            ...
            # If no wall is hit within MAX_CAP, closest remains 200.0 and hit remains None.
            if hit:
                # Confidence is now calculated based on the capped distance
...
class Mouse(DynCircle):
    def __init__(self, space, position, radius, mass, color, maze_lines, offsets):
        super().__init__(mass, radius, 0.5, 0.1, space, color, position)
        self.maze_lines, self.mount_angles, self.offsets = maze_lines, [0, 180, 270, 90], offsets
        self.sensors = [SonarAgent(rel_angle_deg=self.mount_angles[a], arm_length=self.offsets[a]) for a in range(len(self.mount_angles))]
        self.telemetry = []
        
        self.est_x, self.est_y = position
        self.est_theta = self.body.angle
        self.last_l_pos = None
        self.last_r_pos = None

    def update_encoder(self):
        curr_l = self.body.local_to_world((0, -self.radius))
        curr_r = self.body.local_to_world((0, self.radius))

        if self.last_l_pos is not None:
            vec_l = (curr_l[0] - self.last_l_pos[0], curr_l[1] - self.last_l_pos[1])
            vec_r = (curr_r[0] - self.last_r_pos[0], curr_r[1] - self.last_r_pos[1])
            
            f_vec = (math.cos(self.body.angle), math.sin(self.body.angle))
            
            dL = vec_l[0] * f_vec[0] + vec_l[1] * f_vec[1]
            dR = vec_r[0] * f_vec[0] + vec_r[1] * f_vec[1]
            
            L = self.radius * 2 
            d_center = (dL + dR) / 2.0
            phi = (dL - dR) / L 
            
            self.est_x += d_center * math.cos(self.est_theta + phi/2.0)
            self.est_y += d_center * math.sin(self.est_theta + phi/2.0)
            self.est_theta += phi

        self.last_l_pos, self.last_r_pos = curr_l, curr_r

    def update(self, screen, offset):
        super().update(screen, offset)
        self.update_encoder()

        ghost_pos = (int(self.est_x + offset[0]), int(self.est_y + offset[1]))
        pygame.draw.circle(screen, (0, 255, 0), ghost_pos, self.radius, 1)
        
        row = {'position': (self.est_x, self.est_y), 
               'heading': math.degrees(self.est_theta) % 360, 
               'readings': []}
