...
class DFSExplorer:
    ...
    def decide_next(self, node: Node, goal_node: Node):
        # 1. Identify all accessible neighbors that haven't been visited
        unvisited_options = []
        ...
        # 2. Case: Multiple options (Junction) - Use A* to pick the best path
        ...
        # 3. Case: Single option (Hallway) - Skip A* and take the only road
        elif len(unvisited_options) == 1:
            return ('drive', unvisited_options[0])

        # 4. Case: No options (Dead End / Fully Explored Area) - Backtrack
...
class Tree:
    ...
    def _do_scan(self, state):
        ...
        # Mapping for reciprocal/opposite directions
        ...
        for direction, is_wall in wall_data.items():
            ...
            # Reciprocal Update: Update the neighbor's "back side" wall
            neighbor = node.grid_links.get(direction)
            if neighbor:
                opp_dir = opposites[direction]
                neighbor.walls[opp_dir] = is_wall
...
def main():
    ...
    spacing, sx, sy, gx, gy = 90, 5,2, 3,3
    maze_lines = build_synced_maze(sim, map_4, spacing)
